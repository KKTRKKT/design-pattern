...
class LazyThreadSafeSingletonTest {
    ...
    public void instance_exist_only_one() {
        LazyThreadSafeSingleton instance1 = LazyThreadSafeSingleton.getInstance();
        assertEquals(instance1, LazyThreadSafeSingleton.getInstance());
    }

    @Test
    public void safe_multi_threading () throws ExecutionException, InterruptedException {
        ExecutorService executorService = Executors.newFixedThreadPool(4);
        List<Future<Object>> futures = executorService.invokeAll(Arrays.asList(LazyThreadSafeSingleton::getInstance, LazyThreadSafeSingleton::getInstance, LazyThreadSafeSingleton::getInstance, LazyThreadSafeSingleton::getInstance));
